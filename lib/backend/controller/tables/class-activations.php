...
use QuadLayers\WLM\Backend\Controller\Tables\View\List_Table_Activations;
...
class Activations {
	...
	function add_page() {
		...
		$qlwlm_activations_admin = new List_Table_Activations();
		...
		$qlwlm_activations_admin->prepare_items();
		?>
			<div id="<?php echo esc_attr( sanitize_key( __CLASS__ ) ); ?>" class="wrap post-type-shop_order">
				<h2><?php esc_html_e( 'Licenses Activations', 'licenses-manager-for-woocommerce' ); ?></h2>
				...
				?>
					...
					<?php
					$qlwlm_activations_admin->search_box( esc_html__( 'Search', 'licenses-manager-for-woocommerce' ), 's', esc_html__( 'Search by site', 'licenses-manager-for-woocommerce' ) );
					$qlwlm_activations_admin->display()
					?>
				</form>
			</div>
			<?php
	...
	function save_screen_options( $status, $option, $value ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( ! isset( $_GET['qlwlm_activations'] ) ) {
			return $value;
		}
	...
	function menu() {
		$page = add_submenu_page( 'woocommerce', esc_html__( 'Licenses Activations', 'licenses-manager-for-woocommerce' ), esc_html__( 'Licenses Activations', 'licenses-manager-for-woocommerce' ), 'manage_woocommerce', 'qlwlm_activations', array( $this, 'add_page' ) );
